...
- National Dex **001–151** now has a complete HeartGold/SoulSilver “Where to Get”
  guide: version-specific encounters, levels, encounter rates, time/radio/swarm/Safari
  conditions, gifts, statics, fossils, historical Mew distribution, and Gen IV-correct
  evolution routes. Evolution-item links open the matching entry in the Items guide.
- New Collection → **Items** view covers **563 inventory/Key Items introduced in
  Generations I–IV**, selectable across all 16 cartridges from Red/Blue through HGSS.
  It separates found/gift, shop/exchange, Pickup, Pokéwalker, event, and explicitly
  unobtainable methods; TMs/HMs show the move for the selected cartridge.
- Data is reproducibly generated by `npm run data:hgss-kanto` and `npm run data:items`.
  Sources are recorded in the generated files/UI; historically ambiguous event and
  programmed-but-unreleased items have reviewed overrides in `item-locations.ts`.
